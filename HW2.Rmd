...
```{r 2.3.6, echo=FALSE}
lm_chosen = lm(shares ~ (. - n_tokens_content - self_reference_max_shares - weekday_is_saturday - weekday_is_sunday - is_weekend - max_negative_polarity - min_negative_polarity)^2, data=news_articles)

news_articles$yhat = predict(lm_chosen, news_articles)

news_articles = mutate(news_articles, viral = ifelse(news_articles$shares >= log(1400), 1, 0))
