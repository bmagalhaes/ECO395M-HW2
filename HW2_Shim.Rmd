...
rmse_result= data.frame('model 1'= colMeans(rmse_check[1]), 'model 2'=colMeans(rmse_check[2]), 'model 3'=colMeans(rmse_check[3]), 'model 4'=colMeans(rmse_check[4]), 'model 5'=colMeans(rmse_check[5]), 'model_medium'=colMeans(rmse_check[6]), row.names="AVG RMSE")

kable(rmse_result) %>% kable_styling("striped")

```

We can find the variable which is the strongest driver of house prices by assessing how much it improves the out-of-sample RMSE when it is included in the model.
So we test how much the out-of-sample increases when we exclude a certain variable from our model 1, and get the average RMSE by doing the Monte Carlo simulation on different training-test sets(80%-20%) for 100 times.

```{r 2.1.4, echo=FALSE, warning=FALSE}
